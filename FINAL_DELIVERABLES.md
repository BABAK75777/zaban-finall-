...
### `services/api.ts` (Modified Function Only)
```typescript
...
function getBaseUrl(): string {
  ...
  // Fallback: Only use window.location.origin in browser when running dev proxy on same origin
  if (typeof window !== 'undefined' && (import.meta as any)?.env?.DEV) {
    const origin = window.location.origin;
    console.warn(
      '⚠️ VITE_API_URL environment variable is not set.\n' +
      `Using fallback: ${origin} (dev proxy mode)\n` +
      'To fix: Create .env.local in project root with:\n' +
      'VITE_API_URL=http://localhost:3001\n\n' +
  ...
  // Production or missing env: show explicit error
  const errorMsg = 
...
- [ ] Browser console shows `ENV CHECK:` once with `VITE_API_URL: http://localhost:3001`
- [ ] Browser console shows `✅ API CONNECTED { ok: true }`
- [ ] No "VITE_API_URL environment variable is not set" error
- [ ] Green "API Connected" banner appears at top of page

### Port Conflict Handling
- [ ] If port 3001 is in use, backend automatically tries next available port
- [ ] If backend runs on different port, update `.env.local` to match
- [ ] Example: If backend runs on 4000, set `VITE_API_URL=http://localhost:4000` in `.env.local`
- [ ] After updating `.env.local`, restart Vite dev server
- [ ] Re-test: `curl http://localhost:4000/health` returns `{"ok":true}`
- [ ] Re-test: Frontend console shows `✅ API CONNECTED`
