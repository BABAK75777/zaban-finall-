...
export async function requireAuth(req, res, next) {
  // Allow health checks, root endpoint, version endpoint, and auth endpoints without auth
  if (req.path === '/' || req.path === '/health' || req.path === '/healthz' || req.path === '/readyz' || req.path === '/version' || req.path.startsWith('/auth/')) {
    return next();
  }
